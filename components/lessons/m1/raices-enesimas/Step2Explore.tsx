...
export default function Step2Explore({ onComplete, isActive }: LessonStepProps) {
  const {
    ...
    toggleHint,
    hideHint,
  } = useExplorePhases<Phase, RootExample>({
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            <div className="text-center mb-6">
              ...
              <div className="flex justify-center items-center">
                {currentExample.index > 3 && (
                  <sup className="text-2xl font-bold text-purple-600">{currentExample.index}</sup>
                )}
                <span className="text-5xl font-bold text-blue-600">
                  {currentExample.index === 3 ? '∛' : '√'}
                </span>
                <span className="text-4xl font-bold text-green-600">{currentExample.radicand}</span>
              </div>
              {currentExample.index > 3 && (
                <p className="text-sm text-gray-500 mt-2">
                  (raíz {currentExample.index === 4 ? 'cuarta' : 'quinta'})
                </p>
              )}
            </div>
            ...
            <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 mb-4">
              <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3 text-center">
                Piensa: ¿qué número multiplicado {currentExample.index} veces da {currentExample.radicand}?
              </p>
              <div className="flex justify-center items-center gap-2 flex-wrap">
                <span className="font-mono text-xl text-amber-600">?</span>
                {Array.from({ length: currentExample.index - 1 }).map((_, i) => (
                ...
                <span className="font-mono text-xl text-green-600">{currentExample.radicand}</span>
              </div>
              <p className="text-xs text-center text-gray-500 mt-2">
                ({currentExample.index} veces)
              </p>
            </div>
                ...
                <div className="bg-green-50 dark:bg-green-900/30 rounded-xl p-6 border border-green-200 dark:border-green-700">
                  <div className="text-center space-y-3">
                    ...
                    <div className="flex items-center justify-center gap-4">
                      <span className="font-mono text-2xl text-blue-600">{currentExample.symbol}</span>
