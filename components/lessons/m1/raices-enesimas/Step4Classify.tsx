...
type RootType = 'square' | 'cube' | 'fourth' | 'fifth' | 'other';
...
export default function Step4Classify({ onComplete, isActive }: LessonStepProps) {
  ...
  const [selectedType, setSelectedType] = useState<RootType | null>(null);
  const [showFeedback, setShowFeedback] = useState(false);
  const [correctCount, setCorrectCount] = useState(0);
  const [answers, setAnswers] = useState<(RootType | null)[]>(Array(EXPRESSIONS.length).fill(null));
  ...
  const handleSelect = (type: RootType) => {
        ...
        <div className="space-y-6 animate-fadeIn">
          ...
          >
            ...
            <p className={cn(correctCount >= 4 ? 'text-green-700 dark:text-green-300' : 'text-amber-700 dark:text-amber-300')}>
              {correctCount >= 4
                ? 'Identificas muy bien los tipos de raíces'
