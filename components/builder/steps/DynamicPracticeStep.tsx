...
import type { PracticeStep, DynamicStepProps } from '@/lib/builder/types';
import { RichText } from '../primitives';

interface DynamicPracticeStepProps extends DynamicStepProps<PracticeStep> {}

/**
 * DynamicPracticeStep - Renders a practice step from JSON schema
 *
 * Features:
 * - Series of problems with hints
 * - Progress tracking
 * - Immediate feedback with explanations
 * - Results summary at the end
 */
export default function DynamicPracticeStep({
  step,
  isActive,
  onComplete,
}: DynamicPracticeStepProps) {
  const { content } = step;
  ...
  const requiredCorrect = content.requiredCorrect ?? Math.ceil(problems.length * 0.8);
        ...
        <>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
              <RichText content={currentProblem.question} />
            </h3>
              ...
              <div className="bg-amber-50 dark:bg-amber-900/30 rounded-lg p-4 mb-6 animate-fadeIn border border-amber-200 dark:border-amber-700">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-sm text-amber-800 dark:text-amber-200">
                    <RichText content={currentProblem.hint} />
                  </p>
                </div>
              </div>
            )}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-700 dark:text-gray-300">
                      <RichText content={currentProblem.explanation} />
                    </p>
                  </div>
                </div>
              </div>
          ...
          >
            ...
            >
              {correctCount === problems.length
                ? (content.resultMessages?.perfect || '¡Perfecto!')
                : passed
                ? (content.resultMessages?.good || '¡Muy bien!')
                : (content.resultMessages?.needsPractice || '¡Sigue practicando!')}
            </h3>
              ...
              {problems.map((problem, i) => (
                ...
                >
                  ...
                  <span className="text-sm text-gray-700 dark:text-gray-300">
                    <RichText content={problem.question} />
                  </span>
                </div>
              ))}
            </div>
