...
import { ArrowLeft, Check, X, Award, Lightbulb, SkipForward, FlipHorizontal2 } from 'lucide-react';
import {
...
export default function SymmetryGame({
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white dark:bg-gray-900 rounded-xl shadow-lg p-4 sm:p-6 md:p-8">
      ...
      {/* Answer Options */}
      <div
        className={`grid gap-3 sm:gap-4 mb-6 ${problem.options.length === 2 ? 'grid-cols-2' : 'grid-cols-2'}`}
      >
        {problem.options.map((option, index) => {
          let buttonClass =
            'bg-white dark:bg-gray-800 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 hover:border-cyan-400 dark:hover:border-cyan-500';
          ...
          if (feedback.show) {
            ...
            buttonClass =
              'bg-cyan-100 dark:bg-cyan-900/50 text-cyan-900 dark:text-cyan-300 border-cyan-500';
          }
