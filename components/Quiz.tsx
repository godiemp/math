...
export default function Quiz({ questions: allQuestions, level, subject, quizMode = 'zen', difficulty = 'medium' }: QuizProps) {
  ...
  // Wrap content with gradient background for rapidfire mode
  const questionContent = (
    <div className={`max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 ${
      quizMode === 'rapidfire' ? 'animate-fadeIn' : ''
    }`}
    style={quizMode === 'rapidfire' ? {
      ...
      {quizMode === 'rapidfire' && !quizSubmitted && (
        <div className="mb-4">
          {showTimer ? (
            <div className="flex items-center justify-between p-2 bg-gradient-to-r from-purple-50 to-pink-50 dark:from-purple-900/30 dark:to-pink-900/30 rounded border border-purple-200 dark:border-purple-700">
            ...
            <button
              onClick={toggleTimer}
              className="text-xs text-purple-500 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 underline font-semibold"
              title="Mostrar timer"
            >
              ⚡ Mostrar tiempo
            </button>
      ...
      {!quizSubmitted && userAnswer !== null && (
        <div className={`mb-4 p-3 rounded-lg border ${
          ...
          <p className={`text-sm text-center font-semibold ${
            ...
            {quizMode === 'rapidfire' ? '✓ ' : ''}Respuesta seleccionada. Puedes cambiarla antes de enviar el quiz.
          </p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-4">
        ...
        {!quizSubmitted && (
          ...
          >
            {quizMode === 'rapidfire' ? '⚡ ' : ''}Enviar Quiz ({userAnswers.filter(a => a !== null).length} de {quizQuestions.length} respondidas)
          </button>
        )}
      </div>
  ...
  // Return with gradient background for rapidfire mode
  if (quizMode === 'rapidfire') {
    return (
      <div className="min-h-screen bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-600 dark:from-indigo-900 dark:via-purple-900 dark:to-pink-900 py-8 px-4">
        {questionContent}
      </div>
    );
  }
